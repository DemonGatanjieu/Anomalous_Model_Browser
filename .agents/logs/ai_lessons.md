...
## 68. Missing Model URLs in Hash Cache & Asynchronous Path Resolution in Model Source Hub

**The Problem**: When opening the Model Source Hub (模型来源统一中控中心), active workflow models showed empty source URL inputs and "已配置: 0 / N", even when valid `.civitai.info` or `.info` sidecars with official download URLs existed on disk. This occurred because `/anomalous/all_hashes` only extracted `{"hash": ..., "size": ...}` and omitted the parsed `"url"`, while the frontend modal opened synchronously without querying `/anomalous/resolve_paths_to_previews`. Furthermore, the single-item auto-detect function tried direct browser `fetch('https://civitai.com/...')` which fails under firewall/CORS conditions instead of probing local sidecars first, and `api_update_metadata` lacked fallback path resolution when called with unknown folder types or relative subpaths from workflow scope.

**The Solution**:
1. Enrich `/anomalous/all_hashes` payload with `"url": meta.get("source_url") or meta.get("civitai_url") or ""` so models in cache immediately provide URLs.
2. In `ui_model_sources.js`, invoke `resolveWorkflowModelsMetadata` upon opening the modal to resolve active models via `/anomalous/resolve_paths_to_previews` in the background, dynamically populating URLs, platform tags, types, and hashes.
3. In `autoDetectModelSource`, prioritize local sidecar and metadata lookup before falling back to Civitai cloud hash API or search.
4. In `api_update_metadata`, normalize path separators and add fallback model resolution via `_resolve_paths_to_model_info_sync` across all model roots so saving custom URLs to local sidecars from workflow scope works reliably.


## 67. Incomplete Try-Catch Block & Code Duplication in Submodules Silently Halts Extension Registration

**The Problem**: During automated text replacements in `ui_doctor.js`, corrupted line splices created an unclosed `try` block without `catch`/`finally` (`SyntaxError: Missing catch or finally after try`), alongside duplicate loader append statements. Because `main.js` statically imports `ui_sidebar.js` which in turn imports `ui_doctor.js`, the browser's JavaScript engine rejected the entire ES module graph at evaluation time. Consequently, `app.registerExtension()` and the DOM creation logic for `#anomalous-trigger-btn` never ran, leading to the total disappearance of the floating trigger icon and topbar button in ComfyUI without visible error prompts on the canvas.

**The Solution**: Always verify the closure of code blocks (especially `try { ... } catch (error) { ... }`) when performing targeted code refactorings. Validate module ASTs and link the entire frontend dependency tree using Node's experimental VM modules (`node --experimental-vm-modules --input-type=module -e "new vm.SourceTextModule(...)"`) across all frontend files before committing. In addition, ensure `#anomalous-trigger-btn` and `.anomalous-trigger-icon` have robust fallback flex layout styles and clean 1em SVG icon sizing to guarantee immediate visual clarity upon load.

## 66. Unescaped Multiline Regex Literal Leads to ES Module SyntaxError & Disappearing Extension Icon

**The Problem**: During prompt composer assembly logic updates, an accidental newline inside a regex literal (`split(/[,，\n]+/` broke into `/[,，` and `]+/`) created an invalid regular expression token (`SyntaxError: Invalid regular expression: missing /`). Under ES module import rules, any unhandled `SyntaxError` immediately halts module graph compilation. Because `main.js` imports `ui_materials.js` which in turn imports `ui_prompt_composer.js`, the entire extension crashed during initialization before reaching `app.registerExtension()` or `setup()`. This caused the extension entry icon (`#anomalous-trigger-btn`) and topbar entry to completely vanish from ComfyUI without an obvious UI-level trace.

**The Solution**: Keep all character-class regex patterns strictly on a single line (e.g. `split(/[\s,，\n\r]+/)`). To prevent `node -c` false negatives on ES modules, test the real module import chain with `node --input-type=module -e "import('./web/main.js').catch(...)"` to catch syntax and token errors early.

## 65. Missing Module Imports Under Global `try-catch` Masks as Data Fetch Failures

**The Problem**: During visual refactoring of UI cards, if an external dependency (such as `bindMaterialDrag` from `material_drag.js`) is omitted from the file's `import` declarations, calling it inside `createRecipeCard` throws a runtime `ReferenceError`. Because card construction runs inside the global `try { ... } catch (error) { ... }` block of `refreshRecipes()`, this runtime error was swallowed and triggered the generic network failure message (`"无法加载工作流配方。"`), creating the false impression that backend data could not be fetched (even though the topbar filter counter had already successfully computed and displayed `1/1`).

**The Solution**: Ensure every cross-module function called in the component is explicitly imported. Separate network data fetching from DOM list rendering inside `refreshRecipes()`, and log DOM rendering errors with stack traces so unexpected component exceptions do not falsely masquerade as network outages.

## 64. On-Demand Dynamic Hashing & Slash Normalization for Missing Model Recovery

When models are renamed locally and their sidecar `.info` files are deleted, static metadata caches and header inspections may return empty hash sets. Treating size-matching candidates as an immediate `identity_conflict` causes false-negative rejections. Instead, if candidate file size matches the provenance record and the file lacks cached hashes, calculate SHA256 on-demand. Upon cryptographic verification, write an offline inferred `.info` sidecar to persist provenance. Furthermore, always normalize path separators (`\` vs `/`) when indexing and querying workflow metadata (`extra.anomalous_hashes`) to avoid lookup misses across operating systems.
...
## 98. Hidden dialogs must not claim to be active modals

**The Problem**: Queue Prompt, Interrupt, and Escape-to-exit-subgraph stopped working as soon as the plugin loaded, even when its model settings overlay was hidden. Rebinding the commands could not help.

**The Cause**: The reusable settings dialog stayed mounted with `role="dialog"` and `aria-modal="true"`. ComfyUI Frontend's keybinding guard treats any matching element as an open modal without checking ancestor visibility, so command-backed shortcuts were rejected before execution.

**The Practice**: Keep reusable dialog DOM if needed, but synchronize its modal semantics with visibility: use `aria-modal="false"` while hidden and switch to `true` only for the visible lifetime. Do not work around host command guards with duplicate global keyboard listeners; fix the incorrect UI state at its source and verify every open/close path.

## 99. Browsable does not mean physically renameable

**The Problem**: Treating every scanned category as a physical-rename target changed stable VAE filenames and made existing workflows report missing models.

**The Practice**: Keep VAE, VAE Approx, CLIP/Text Encoder, and CLIP Vision available for browsing, metadata scans, and virtual naming while denying physical rename in the detail UI, scan APIs, metadata API, and standalone scraper. A safety policy needs an authoritative backend guard even when the current UI already disables the action.

## 100. A virtual name must own the primary browsing surface

**The Problem**: Scanning correctly wrote `anomalous_custom_name`, and model detail displayed it, but the main grid always rendered the physical filename. Users reasonably concluded that virtual rename had failed, and searching the visible name could not find the card.

**The Practice**: Treat the virtual/custom name as the card title, retain the physical filename as a smaller diagnostic subtitle, and search physical paths before falling back to sidecar display names. Keep the full absolute path in model detail with a copy action; folder-tree navigation already owns relative location context.

## 101. Foundation-component recovery is hash-only

**The Problem**: VAE and text-encoder files often keep ecosystem-stable names while Civitai and Hugging Face publish different labels. Size-only recovery could silently select a different foundational component, while blocking an existing same-named file would disagree with ComfyUI's own valid combo state.

**The Practice**: If the native filename exists, keep it usable and surface a non-blocking warning only when both workflow and local hashes exist but differ. If the filename is missing, VAE, VAE Approx, CLIP/Text Encoder, and CLIP Vision may recover only from one exact in-category workflow hash. Without that hash, leave the reference unresolved for manual handling; never guess from size or display names.

## 102. A lock file is evidence, not proof of a live job

**The Problem**: Folder and global scans wrote a plain marker before starting a daemon thread and removed it only from the thread's `finally`. Closing ComfyUI during a scan could terminate the daemon before cleanup, so every later session treated the leftover file as a permanently active scan. Reinstalling appeared to be the only recovery because it happened to remove the plugin-local global marker.

**The Practice**: Persist enough ownership to validate a background job: version, backend session, owner PID, worker PID, and job ID. A status/start request must preserve a genuinely live worker but atomically clear markers and progress artifacts owned by dead or legacy sessions. Pair that recovery contract with real, atomically written progress data; a spinner or bare file-existence flag cannot tell the user whether work is advancing.

## 103. Preserving core navigation buttons and enforcing sharp dark-crimson styling

**The Problem**: During top-bar refactoring and visual updates, the sidebar docking button (`dockBtn`) was accidentally omitted from the header's right action group. Concurrently, button hover halos were over-diffused (large blur radius resembling blurry flashlights), panel backdrops lacked fallback styling and turned pitch-black when image assets were absent, and residual cold-blue/slate-gray gradients polluted the intended dark gothic crimson aesthetic.

**The Practice**:
1. **Never drop persistent navigation actions during refactoring**: Core actions like `#anomalous-dock-btn` must be preserved in header DOM groups alongside proper localization and tooltips (`[data-tooltip-pos="bottom"]`).
2. **Constrain button hover glows**: For premium dark/crimson themes, constrain `:hover` blur radii to 2px–4px (`box-shadow: 0 0 4px 1px ...`) to achieve tight, razor-sharp magic rim glows rather than muddy flashlight halos.
3. **Always supply CSS gradient fallbacks for conceptual backdrops**: Pair image backdrops with dark crimson radial gradients (`background-image: url('...'), radial-gradient(...)`) and base colors (`#0d080c`) so missing or slow-loading assets never degrade into dead black voids.
4. **Purge cold slate tones from dark crimson palettes**: Enforce pure dark tones tinted with deep crimson (`linear-gradient(to top, rgba(15, 5, 10, 0.95), transparent)`) across card title masks, drawer inputs, and containers.
## 104. Header drag delegation must guard closest interactive elements, and model card palettes must strictly harmonize with Gothic Sovereign colors

**The Problem**:
1. When header buttons contain nested SVGs or spans (e.g. `#anomalous-notebook-btn`), checking `e.target.tagName === 'BUTTON'` in `header.addEventListener('mousedown')` fails whenever users click text or icons (`e.target` is `SPAN`, `SVG`, or `PATH`). The handler treated the interaction as a header drag, set `isDragging = true`, and executed `e.preventDefault()`, which prevented the browser from firing the subsequent `click` event and rendered the button completely unresponsive.
2. Model cards featured hardcoded raw cyan badges (`#00ffcc`), severely clashing with the dark crimson/wine Gothic Vampire aesthetic.
3. The main gallery/grid area suffered from a "dead pitch-black void" due to flat `#000000` / `#0d080c` fills with overly dark overlay blending and no ambient illumination behind floating cards.

**The Practice**:
1. **Interactive element guard in drag handlers**: Always use `e.target.closest('button') || e.target.closest('input') || e.target.closest('select') || e.target.closest('textarea') || e.target.closest('.anomalous-header-close')` in mousedown drag handlers to guarantee that clicks on nested child icons and text spans are never intercepted or suppressed.
2. **Harmonize badge palettes (Gothic Sovereign Palette)**: Replace cold cyan badges with semantic Gothic classes: Dark Gold (`#f59e0b`), Amber (`#fbbf24`), Abyssal Purple (`#c084fc`), and Rose Crimson (`#ff4d6d`), strictly eliminating jarring neon cyan.
3. **Eliminate dead black voids with multi-layered ambient lighting**: Build depth behind floating cards using deep dark wine obsidian bases (`#0b050f` / `#0c0510`), top-centered radial crimson moon glow (`radial-gradient(ellipse ... rgba(190, 20, 55, 0.12)...)`), soft-light concept mansion backing (`mix-blend-mode: soft-light`), and custom gothic crimson-accented scrollbars.

## 105. Precise safe-corridor geometry and blank-space dismiss guards for popover tooltips

**The Problem**:
1. In the Prompt Studio sidebar, `isPointInSafeZone` used a coarse bounding rectangle (`minX` to `maxX`, `minY` to `maxY`) between the anchor card and the floating popover. When a card preview popover was tall, this bounding rectangle spanned the entire width of the left column (0~190px) across hundreds of vertical pixels, falsely treating the list's empty space as part of the bridge corridor and continuously resetting the hide timer (`cancelHidePopover()`), resulting in a zombie/stuck popover.
2. CSS added a 28px wide transparent `::before` pseudo-element on the popover that penetrated 22px into the left panel, capturing pointer events and triggering `popover.onmouseenter` over whitespace.
3. No explicit dismiss handling was attached to blank container areas or outside pointerdowns when unpinned.

**The Practice**:
1. **Never use full bounding boxes for separate columns**: Safe corridors must be strictly bounded to the physical gap between trigger and target (`card.right - 6` to `popover.left + 6`), ensuring the pointer immediately leaves the safe zone when moving into whitespace above, below, or to the side.
2. **Differentiate dismiss timing**: Use snappy delays (80~100ms) when the pointer moves outside the safe zone or into whitespace, while maintaining standard tolerance (200~250ms) for deliberate transitions into the popover.
3. **Empty space & outside click dismissal**: Containers must explicitly trigger fast hide on whitespace hover/movement and immediately destroy unpinned popovers on whitespace or outside clicks.
4. **Avoid oversized invisible CSS hitboxes**: Prefer bounded mathematical hit testing in JavaScript over large transparent pseudo-elements (`::before`) that bleed across panel boundaries.
