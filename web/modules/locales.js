...
<div style="line-height: 1.6; font-size: 0.95em; color: #eee; padding: 10px;">
    ...
    <p><strong>3. 🤖 选中交互 (节点助手)</strong><br>具体位置：左侧第三个 <strong>机器人 (🤖)</strong> 按钮。<br>打开后在画布上选择模型节点。顶部操作栏可以替换当前模型，也可在兼容的 MODEL + CLIP 链前方或后方插入 LoRA。选择器会自动限定节点原生支持的模型类型，并提供文件夹、完整路径搜索和排序；多分支链路有歧义时不会自动改线。</p>
...
<div style="line-height: 1.6; font-size: 0.95em; color: #eee; padding: 10px;">
    ...
    <p><strong>3. 🤖 Visual Info (Node Assistant)</strong><br>Location: The 3rd icon <strong>(Robot 🤖)</strong> from the bottom left.<br>Select a model node to replace its current model or insert a LoRA before/after a compatible MODEL + CLIP chain. The picker is limited to the node's native model type and includes folder browsing, full-path search, and sorting. Ambiguous multi-branch chains are not rewired automatically.</p>
