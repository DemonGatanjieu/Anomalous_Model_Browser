...
class AnomalousBrowser {
    ...
    createDOM() {
        ...
        menuBtn.innerHTML = '鈽?;
        menuBtn.title = window.anomalous_browser_lang === 'zh' ? '鏀惰捣/灞曞紑渚ц竟鏍? : 'Toggle Sidebar';
        ...
        modelsBtn.innerHTML = `馃彔 <span class="anomalous-btn-text">${t('models')}</span>`;
        modelsBtn.onclick = () => {
            this.hideAllPanels();
        ...
        galleryBtn.innerHTML = `馃柤锔?<span class="anomalous-btn-text">${t('gallery') || '鍥惧簱'}</span>`;
        galleryBtn.onclick = () => {
            this.hideAllPanels();
            container.classList.add('anomalous-sidebar-closed');
        ...
        helpBtn.innerHTML = `鉂?<span class="anomalous-btn-text">${t('help')}</span>`;
        helpBtn.onclick = () => this.showHelp();

        const nbBtn = document.createElement('button');
        nbBtn.id = 'anomalous-notebook-btn';
        nbBtn.title = t('notebookTitle');
        nbBtn.innerHTML = `馃搼 <span class="anomalous-btn-text">${t('notebooks')}</span>`;

        const dBtn = document.getElementById('anomalous-doctor-btn');
        if (dBtn) dBtn.title = window.anomalous_browser_lang === 'zh' ? '妯″瀷鍖荤敓' : 'Model Doctor';
        const aBtn = document.getElementById('anomalous-assistant-btn');
        if (aBtn) aBtn.title = window.anomalous_browser_lang === 'zh' ? '鑺傜偣鍔╂墜' : 'Node Assistant';
        const iBtn = document.getElementById('anomalous-import-btn');
        if (iBtn) iBtn.title = window.anomalous_browser_lang === 'zh' ? '馃摜 棰勬瀵煎叆宸ヤ綔娴? : '馃摜 Preflight Import';
        const sBtn = document.getElementById('anomalous-settings-btn');
        if (sBtn) sBtn.title = window.anomalous_browser_lang === 'zh' ? '璁剧疆涓績' : 'Settings Hub';
        ...
        scanBtn.title = window.anomalous_browser_lang === 'zh' ? '馃攧 鎵弿鍚戝 (Scan Wizard)' : '馃攧 Scan Wizard';
        scanBtn.innerHTML = `馃攧`;
        ...
        setInterval(async () => {
            try {
                ...
                if (isScanning && !isCurrentlyScanning) {
                    ...
                    alert(window.anomalous_browser_lang === 'zh' ? '鉁?鎵弿/閲嶅懡鍚嶅凡瀹屾垚锛佹暟鎹凡涓烘偍鏇存柊銆? : '鉁?Scan/Rename completed! Data updated.');
                }
            } catch (e) { }
        }, 3000);
        ...
        const createWizardModal = (isGlobal = false) => {
            ...
            const apiKeyBtn = createGhostBtn('馃攽', 'API 瀵嗛挜', 'API Key', () => {
                const currentKey = localStorage.getItem('anomalous_civitai_api_key') || '';
                const newKey = prompt(window.anomalous_browser_lang === 'zh' ? '璇疯緭鍏?Civitai API Key (鐣欑┖鍙栨秷):' : 'Enter Civitai API Key:', currentKey);
                if (newKey !== null) {
                    localStorage.setItem('anomalous_civitai_api_key', newKey.trim());
                    alert(window.anomalous_browser_lang === 'zh' ? '鉁?瀵嗛挜淇濆瓨鎴愬姛' : '鉁?Key Saved');
                }
            });
            ...
            sec1Title.innerText = window.anomalous_browser_lang === 'zh' ? 'Step 1. 鑾峰彇鏁版嵁' : 'Step 1. Fetch Data';
            ...
            const updateCards = () => {
                ...
                card1 = createChoiceCard('civitai', '馃實', '鍦ㄧ嚎瀹屾暣鍖归厤', 'Online Full Matching', '鑷姩杩炴帴 Civitai 骞冲彴锛屾繁搴﹁幏鍙栨ā鍨嬪皝闈㈠浘銆佽Е鍙戣瘝 (Trigger Words)銆佷綔鑰呭娉ㄥ強璇︾粏鏍囩銆傛帹鑽愭棩甯镐娇鐢ㄣ€?, 'Connects to Civitai to fetch comprehensive model covers, trigger words, author notes, and tags. Recommended for daily use.', scanMode === 'civitai');
                card2 = createChoiceCard('offline', '馃攲', '绂荤嚎鏋侀€熻鍙?, 'Offline Fast Read', '涓嶆秷鑰椾换浣曠綉缁溿€備粎閫氳繃鏈湴寮犻噺璁＄畻鏋侀€熸彁鍙栨ā鍨嬪唴鐨勫熀纭€鍏冩暟鎹紝閫熷害鏋佸揩浣嗕俊鎭緝鍩虹銆?, 'No network required. Extremely fast extraction of basic metadata via local tensor computation, but info is limited.', scanMode === 'offline');
            ...
            sec2Title.innerText = window.anomalous_browser_lang === 'zh' ? 'Step 2. 瑙勮寖鍛藉悕' : 'Step 2. Normalize Naming';
            ...
            vRow.style.gap = '8px';
            vRow.innerHTML = `<span style="font-size:0.9em; color:#ddd;">鉁?${window.anomalous_browser_lang === 'zh' ? '铏氭嫙閲嶅懡鍚?(瀹夊叏鎺ㄨ崘)' : 'Virtual Rename (Safe)'}</span>`;
            ...
            vDesc.innerHTML = window.anomalous_browser_lang === 'zh' ? '浠呭皢鏍囧噯鍚嶇О娉ㄥ叆娴忚鍣ㄦ彃浠朵腑锛屼笉淇敼纭洏涓婄殑鐪熷疄鏂囦欢鍚嶏紝闅忔椂鍙棤鎹熻繕鍘熴€? : 'Inject standard names into the browser only, without modifying actual files on disk. Safely reversible.';
            ...
            pRow.style.gap = '8px';
            pRow.innerHTML = `<span style="font-size:0.9em; color:#ddd;">馃捑 ${window.anomalous_browser_lang === 'zh' ? '鐗╃悊閲嶅懡鍚? : 'Physical Rename'}</span>`;
            ...
            pDesc.innerHTML = window.anomalous_browser_lang === 'zh' ? '鐪熷疄淇敼搴曞眰鐨?<code>.safetensors</code> 鍙婂叾鎵€鏈夐檮灞炴枃浠跺悕锛屽交搴曞憡鍒贡鐮佹枃浠跺悕銆? : 'Permanently rename the underlying <code>.safetensors</code> and associated files on disk.';
            ...
            sec3Title.innerText = window.anomalous_browser_lang === 'zh' ? 'Step 3. 宸ヤ綔娴佷繚闅滀笌淇' : 'Step 3. Workflow Protection & Fix';
            ...
            s3List.appendChild(createListRow('馃摝', '妯″瀷婧簮缁戝畾', 'Model Provenance Binding', '鍏ㄥ眬鐢熸晥銆傛瘡娆＄敓鎴愬浘鐗囨垨淇濆瓨宸ヤ綔娴佹椂锛屽皢妯″瀷鐨勭簿纭搱甯屽€奸殣寮忓啓鍏ュ埌鍥惧儚鍏冩暟鎹笌宸ヤ綔娴?JSON 涓紝闃叉鏈潵妯″瀷鏀瑰悕鎴栨崲鐜鍚庢姤閿欐壘涓嶅埌銆?, 'Global effect. Implicitly embed exact model hashes into generated images and saved workflows, ensuring missing models can always be auto-fixed.', createMaterialSwitch(isInject, (s) => localStorage.setItem('anomalous_inject_hash', s ? 'true' : 'false'))));
            s3List.appendChild(createListRow('馃獎', '鏅鸿兘淇褰撳墠宸ヤ綔娴?, 'Smart Fix Current Workflow', '鎵弿瀹屾垚鍚庯紝鑷姩灏濊瘯鐢ㄦ渶鏂版暟鎹慨澶嶅綋鍓嶇敾甯冧腑鎻愮ず缂哄け鐨勬姤閿欐ā鍨嬭妭鐐广€?, 'After scanning, automatically attempt to fix missing model errors on the current canvas.', createMaterialSwitch(enableAutoCheck, (s) => enableAutoCheck = s)));
            ...
            const doScan = async () => {
                try {
                    ...
                    if (data.status === 'ok') {
                        ...
                        alert(window.anomalous_browser_lang === 'zh' ? '鎵弿澶辫触: ' + data.message : 'Scan failed: ' + data.message);
                    }
            ...
            closeBtn.innerText = window.anomalous_browser_lang === 'zh' ? '鍙栨秷' : 'Cancel';
            ...
            startBtn.innerText = window.anomalous_browser_lang === 'zh' ? '鎵ц鎵弿' : 'Execute Scan';
        ...
        const renderEnergyBtn = () => {
            energyBtn.innerHTML = this.energySaving
                ? `<span class="anomalous-btn-text">${(window.anomalous_browser_lang === 'zh' ? '馃攱 瑙嗛鎮诞鎾斁 (鑺傝兘妯″紡)' : '馃攱 Eco Mode (Play video on hover)')}</span>`
                : `<span class="anomalous-btn-text">${(window.anomalous_browser_lang === 'zh' ? '馃幀 鑷姩鎾斁瑙嗛灏侀潰 (楂樿兘鑰?' : '馃幀 AutoPlay Videos (High perf cost)')}</span>`;
        };
        renderEnergyBtn();
        energyBtn.onclick = () => {
        ...
        cleanBtn.onclick = async () => {
            ...
            cleanBtn.innerHTML = `鈴?<span class="anomalous-btn-text">${t('cleaning')}</span>`;
        ...
        langBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃寪 Language: EN' : '馃寪 璇█: 涓枃';
        langBtn.onclick = () => {
            ...
            updateLangClass();
            modelsBtn.innerHTML = `馃彔 <span class="anomalous-btn-text">${t('models')}</span>`;
            galleryBtn.innerHTML = `馃柤锔?<span class="anomalous-btn-text">${t('gallery')}</span>`;
            ...
            if (globalScanBtnRef) globalScanBtnRef.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃實 涓€閿叏鐩樻瀬閫熸壂鎻?(涓嶄笅杞藉皝闈?涓嶆敼鍚?' : '馃實 Global Quick Scan (No rename/No media)';
            const checkUnscannedBtnRef = document.getElementById('anomalous-check-unscanned-btn');
            if (checkUnscannedBtnRef) checkUnscannedBtnRef.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攳 妫€鏌ュ苟鏋侀€熷綍鍏ョ己澶辨ā鍨嬩俊鎭? : '馃攳 Check & Auto-Scan Missing Info';
            const resetBtnRef = document.getElementById('anomalous-reset-btn');
            if (resetBtnRef) resetBtnRef.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攧 閲嶇疆鐣岄潰甯冨眬' : '馃攧 Reset Layout';
            const scaleLabelRef = document.getElementById('anomalous-scale-label');
            if (scaleLabelRef) scaleLabelRef.innerText = window.anomalous_browser_lang === 'zh' ? 'UI 缂╂斁' : 'UI Scale';
        ...
        globalScanBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃實 涓€閿叏鐩樻瀬閫熸壂鎻?(涓嶄笅杞藉皝闈?涓嶆敼鍚?' : '馃實 Global Quick Scan (No rename/No media)';
        styleHubBtn(globalScanBtn);

        globalScanBtn.onclick = async () => {
            if (!confirm(window.anomalous_browser_lang === 'zh' ? '鍗冲皢鎵ц鏋侀€熷叏鐩樻壂鎻忓苟鍚屾Hash锛屾鎿嶄綔涓嶆敼鍚嶄篃涓嶄笅杞藉皝闈紝鏄惁缁х画锛? : 'Start global quick scan to sync all hashes?')) return;
            globalScanBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鈴?鎵弿涓?..' : '鈴?Scanning...';
            ...
            try {
                ...
                const data = await res.json();
                if (data.status === 'ok') {
                    alert(window.anomalous_browser_lang === 'zh' ? '馃殌 鍏ㄥ眬鎵弿宸插悗鍙板惎鍔紒' : '馃殌 Global Scan started in background!');
                    const pollTimer = setInterval(async () => {
                        try {
                            ...
                            if (!statusData.scanning) {
                                ...
                                globalScanBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鉁?鎵弿瀹屾垚' : '鉁?Scan Complete';
                                setTimeout(() => {
                                    globalScanBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃實 涓€閿叏鐩樻壂鎻?(涓嶄笅杞藉皝闈?涓嶆敼鍚?' : '馃實 Global Quick Scan (No rename/No media)';
                    ...
                    alert((window.anomalous_browser_lang === 'zh' ? '閿欒: ' : 'Error: ') + data.message);
                    globalScanBtn.disabled = false;
                }
            } catch (e) {
                globalScanBtn.disabled = false;
        ...
        const renderLocalParseToggleBtn = () => {
            ...
            localParseToggleBtn.innerHTML = window.anomalous_browser_lang === 'zh'
                ? (isLocalEnabled ? '鈽?寮€鍚潪C绔欐ā鍨嬫湰鍦版壂鎻?(璇诲彇鍐呯疆鍙傛暟) [宸插紑鍚痌' : '鈽?寮€鍚潪C绔欐ā鍨嬫湰鍦版壂鎻?(璇诲彇鍐呯疆鍙傛暟) [宸插叧闂璢')
                : (isLocalEnabled ? '鈽?Local metadata scan for non-Civitai [ON]' : '鈽?Local metadata scan for non-Civitai [OFF]');
        };
        ...
        checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攳 妫€鏌ュ苟鏋侀€熷綍鍏ョ己澶辨ā鍨嬩俊鎭? : '馃攳 Check & Auto-Scan Missing Info';
        styleHubBtn(checkUnscannedBtn);
        checkUnscannedBtn.onclick = async () => {
            checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鈴?妫€鏌ヤ腑...' : '鈴?Checking...';
            ...
            try {
                ...
                if (hasUnscanned) {
                    checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鈿狅笍 鍙戠幇缂哄け锛屾鍦ㄨ嚜鍔ㄦ瀬閫熸壂鎻?..' : '鈿狅笍 Missing info found, Auto-Scanning...';
                    ...
                    if (scanData.status === 'ok') {
                        const pollTimer = setInterval(async () => {
                            try {
                                ...
                                if (!statusData.scanning) {
                                    clearInterval(pollTimer);
                                    checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鉁?琛ュ叏瀹屾垚' : '鉁?Info Complete';
                                    setTimeout(() => {
                                        checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攳 妫€鏌ュ苟鏋侀€熷綍鍏ョ己澶辨ā鍨嬩俊鎭? : '馃攳 Check & Auto-Scan Missing Info';
                        ...
                        alert((window.anomalous_browser_lang === 'zh' ? '閿欒: ' : 'Error: ') + scanData.message);
                        checkUnscannedBtn.disabled = false;
                        checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攳 妫€鏌ュ苟鏋侀€熷綍鍏ョ己澶辨ā鍨嬩俊鎭? : '馃攳 Check & Auto-Scan Missing Info';
                    }
                } else {
                    checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鉁?鎵€鏈夋ā鍨嬩俊鎭凡瀹屾暣' : '鉁?All Model Info is Complete';
                    setTimeout(() => {
                        checkUnscannedBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攳 妫€鏌ュ苟鏋侀€熷綍鍏ョ己澶辨ā鍨嬩俊鎭? : '馃攳 Check & Auto-Scan Missing Info';
                        checkUnscannedBtn.disabled = false;
                    }, 3000);
        ...
        scaleLabel.innerText = window.anomalous_browser_lang === 'zh' ? 'UI 缂╂斁' : 'UI Scale';
        ...
        resetBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃攧 閲嶇疆鐣岄潰甯冨眬' : '馃攧 Reset Layout';
        styleHubBtn(resetBtn);
        resetBtn.onclick = () => {
            if (confirm(window.anomalous_browser_lang === 'zh' ? '纭閲嶇疆绐楀彛浣嶇疆銆佺缉鏀惧拰鍋滈潬鐘舵€佸悧锛? : 'Reset window position, scale and dock state?')) {
        ...
        const updateHashToggleBtn = () => {
            ...
            hashToggleBtn.innerHTML = window.anomalous_browser_lang === 'zh'
                ? (isInject ? '馃煝 娉ㄥ叆宸ヤ綔娴佸搱甯? : '鈿?涓嶆敞鍏ュ伐浣滄祦鍝堝笇')
                : (isInject ? '馃煝 Inject Workflow Hash' : '鈿?Skip Workflow Hash');
        };
        updateHashToggleBtn();
        ...
        settingsBtn.innerHTML = `鈿欙笍`;
        settingsBtn.title = window.anomalous_browser_lang === 'zh' ? '璁剧疆涓績' : 'Settings Hub';
        ...
        importBtn.onclick = () => {
            if (window.AMB_WorkflowShare) {
                ...
                alert(window.anomalous_browser_lang === 'zh' ? '妯″潡灏氭湭鍔犺浇锛岃绋嶇瓑鎴栧埛鏂伴〉闈紒' : 'Module not loaded, please refresh!');
            }
        };
        ...
        doctorBtn.title = window.anomalous_browser_lang === 'zh' ? '妯″瀷鍖荤敓' : 'Model Doctor';
        doctorBtn.innerHTML = `馃┖`;
        ...
        assistantBtn.title = window.anomalous_browser_lang === 'zh' ? '鑺傜偣鍔╂墜' : 'Node Assistant';
        assistantBtn.innerHTML = `馃`;
    ...
    renderSidebar() {
        ...
        collapseAllBtn.innerHTML = isAllCollapsed ? (window.anomalous_browser_lang === 'zh' ? '鉃?灞曞紑鍏ㄩ儴' : '鉃?Expand All') : (window.anomalous_browser_lang === 'zh' ? '鉃?鏀惰捣鍏ㄩ儴' : '鉃?Collapse All');
        ...
        searchInput.placeholder = window.anomalous_browser_lang === 'zh' ? '馃攳 鎼滅储妯″瀷...' : '馃攳 Search models...';
        ...
        (this.foldersData || []).forEach(typeGroup => {
            ...
            header.innerHTML = `<span>${typeGroup.label}</span> <span>${isTypeExpanded ? '鈻? : '鈻?}</span>`;
            ...
            sortedPaths.forEach(path => {
                ...
                let toggleIcon = '';
                if (hasChildren) {
                    toggleIcon = `<span class="anomalous-folder-toggle" style="margin-right: 8px; width: 12px; display: inline-block; font-size: 0.8em; color: #888;">${isExpanded ? '鈻? : '鈻?}</span>`;
                } else {
    ...
    async loadModels() {
        try {
            ...
            data.models.forEach(model => {
                ...
                editBtn.innerHTML = '鈿欙笍';
                editBtn.title = window.anomalous_browser_lang === 'zh' ? '缂栬緫妯″瀷 (Edit)' : 'Edit Model';
    ...
    applyModelToCanvas(type, subfolder, model) {
        ...
        if (!nodeType) {
            alert(window.anomalous_browser_lang === 'zh' ? '涓嶆敮鎸佽绫诲瀷妯″瀷鐨勮嚜鍔ㄥ簲鐢ㄣ€? : 'Unsupported model type for auto-apply.');
            return;
        }

        const node = LiteGraph.createNode(nodeType);
        if (!node) {
            alert((window.anomalous_browser_lang === 'zh' ? '鍒涘缓鑺傜偣澶辫触: ' : 'Failed to create node: ') + nodeType);
            return;
        }
        ...
        const stickHandler = (e) => {
            ...
            // LiteGraph鍐呯疆浜嗗潗鏍囪浆鎹紝瀹冧細瀹岀編澶勭悊缂╂斁鍜屽亸绉诲甫鏉ョ殑鍧愭爣鍋忕Щ闂
            let canvasX, canvasY;
    ...
    showDetail(model) {
        ...
        if (isFromDoctor) {
            backBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '猬?杩斿洖鍖荤敓闈㈡澘' : '猬?Back to Doctor';
            backBtn.style.background = '#8AB4F8';
            backBtn.style.color = '#000';
        } else if (isFromAssistant) {
            backBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '猬?杩斿洖鍔╂墜闈㈡澘' : '猬?Back to Assistant';
            backBtn.style.background = '#8AB4F8';
            backBtn.style.color = '#000';
        } else {
        ...
        title.style.flex = '1'; // 鎾戝紑鍓╀綑绌洪棿锛屾妸鍙充晶鎸夐挳鎸ゅ埌鏈€鍙宠竟
        ...
        editMetaBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鈿欙笍 缂栬緫' : '鈿欙笍 Edit';
        ...
        if (m.custom_notes) {
            ...
            notesTitle.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃摀 涓撳睘澶囨敞 (Notes)' : '馃摀 Custom Notes';
            ...
            notesEditBtn.innerHTML = '鉁忥笍';
            notesEditBtn.title = window.anomalous_browser_lang === 'zh' ? '缂栬緫澶囨敞' : 'Edit Notes';
        ...
        galleryBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃柤锔?鏌ョ湅鍘嗗彶鐢熸垚浣滃搧' : '馃柤锔?View Generated Gallery';
        ...
        if (m.baseModel) {
            ...
            compTitle.innerHTML = `${t('compatibleModels') || '馃敆 Compatible'} <span style="font-size:0.8em; opacity:0.6;">(${m.baseModel})</span>`;
    ...
    async showNotebooks() {
        ...
        createBtn.innerHTML = `鉃?<span class="anomalous-nb-create-text">${t('createNotebook')}</span>`;
    ...
    async loadGalleryImages(page = 1, reset = false) {
        ...
        this.gallerySentinel.innerHTML = window.anomalous_browser_lang === 'zh' ? '鍔犺浇涓?..' : 'Loading...';
        ...
        try {
            ...
            if (data.images && data.images.length > 0) {
                data.images.forEach(imgData => {
                    ...
                    delBtn.innerHTML = '馃棏锔?;
                    delBtn.title = window.anomalous_browser_lang === 'zh' ? '鍒犻櫎' : 'Delete';
                    ...
                    delBtn.onclick = (e) => {
                        ...
                        msg.innerHTML = '褰诲簳鍒犻櫎杩欏紶鍥剧墖锛?br><span style="font-size:0.8em;color:#aaa">Delete permanently?</span>';
                        msg.style.color = '#fff';
                        ...
                        confirmBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '馃棏锔?鍒犻櫎' : '馃棏锔?Delete';
                        ...
                        cancelBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鍙栨秷' : 'Cancel';
                        ...
                        confirmBtn.onclick = async (ce) => {
                            ce.stopPropagation();
                            confirmBtn.innerHTML = window.anomalous_browser_lang === 'zh' ? '鍒犻櫎涓?..' : 'Deleting...';
                            ...
                            try {
                                ...
                                if (dd.status === 'success') {
                                    ...
                                    alert((window.anomalous_browser_lang === 'zh' ? '鍒犻櫎澶辫触: ' : 'Delete failed: ') + dd.message);
                                    overlay.remove();
                                }
                            } catch (err) {
                                alert((window.anomalous_browser_lang === 'zh' ? '閿欒: ' : 'Error: ') + err);
                                overlay.remove();
                            }
                        };
                ...
                if (!this.galleryHasMore) {
                    this.gallerySentinel.innerHTML = window.anomalous_browser_lang === 'zh' ? '娌℃湁鏇村鍥剧墖浜? : 'No more images';
                } else {
                    this.gallerySentinel.innerHTML = window.anomalous_browser_lang === 'zh' ? '鍚戜笅婊氬姩鍔犺浇鏇村' : 'Scroll for more';
                }
            } else {
                this.galleryHasMore = false;
                this.gallerySentinel.innerHTML = reset ? '鍥惧簱涓虹┖ / Gallery is empty' : '娌℃湁鏇村鍥剧墖浜?/ No more images';
            }
        } catch (e) {
            console.error('Failed to load gallery images', e);
            this.gallerySentinel.innerHTML = window.anomalous_browser_lang === 'zh' ? '鍔犺浇澶辫触' : 'Load failed';
        }
    ...
    showEditModal(model) {
        ...
        if (model.preview_url) {
            ...
            if (isVideo) {
                ...
                muteBtn.title = window.anomalous_browser_lang === 'zh' ? '寮€鍚?鍏抽棴澹伴煶' : 'Toggle Sound';
                muteBtn.onclick = (e) => {
                    e.stopPropagation();
                    video.muted = !video.muted;
                    muteBtn.innerHTML = video.muted ? '馃攪' : '馃攰';
                };
            ...
            previewContainer.innerHTML = `<div style="color:#9aa0a6; font-size:0.9em; text-align:center;">${window.anomalous_browser_lang === 'zh' ? '鏆傛棤灏侀潰' : 'No Cover'}</div>`;
        }
        ...
        const galleryBtn = document.createElement('button');
        galleryBtn.innerHTML = '馃柤锔?' + (window.anomalous_browser_lang === 'zh' ? '浠庡巻鍙插浘搴撴寫閫? : 'Pick from Gallery');
        ...
        localBtn.innerHTML = '馃搧 ' + (window.anomalous_browser_lang === 'zh' ? '浠庢湰鍦颁笂浼? : 'Upload Local');
            ...
            fileInput.onchange = async (e) => {
                if (e.target.files && e.target.files.length > 0) {
                    ...
                    try {
                        ...
                        alert((window.anomalous_browser_lang === 'zh' ? '涓婁紶澶辫触: ' : 'Upload failed: ') + err);
                    }
                }
            };
            fileInput.click();
        ...
        const title = document.createElement('h2');
        title.innerText = window.anomalous_browser_lang === 'zh' ? '缂栬緫妯″瀷淇℃伅' : 'Edit Model Info';
        title.style.margin = '0';
        ...
        filenameLabel.innerHTML = `<span style="color:#9aa0a6;">${window.anomalous_browser_lang === 'zh' ? '鏂囦欢:' : 'File:'}</span> ${model.filename}`;
        ...
        nameInput.placeholder = window.anomalous_browser_lang === 'zh' ? '鑷畾涔夊悕绉?(鐣欑┖鍒欎娇鐢ㄥ師鍚?' : 'Custom Name (Leave empty to use original)';
        ...
        notesInput.placeholder = window.anomalous_browser_lang === 'zh' ? '馃摀 涓撳睘澶囨敞... (鏀寔澶氳)' : '馃摀 Custom Notes...';
        ...
        physicalLabel.innerText = window.anomalous_browser_lang === 'zh' ? '鍚屾鐗╃悊閲嶅懡鍚? : 'Physical Rename on Disk';
        ...
        physicalDesc.innerText = window.anomalous_browser_lang === 'zh' ? '鑻ヤ笉鍕鹃€夊垯浠呰櫄鎷熼噸鍛藉悕锛屽畨鍏ㄤ笉鐮村潖鍘熸枃浠躲€? : 'Virtual rename only if unchecked. Safe for originals.';
        ...
        const resetBtn = document.createElement('button');
        resetBtn.innerText = window.anomalous_browser_lang === 'zh' ? '閲嶇疆璁剧疆' : 'Reset All';
        ...
        resetBtn.onclick = async () => {
            if (!confirm(window.anomalous_browser_lang === 'zh' ? '纭畾瑕侀噸缃妯″瀷鐨勬墍鏈夎嚜瀹氫箟淇℃伅锛堝悕绉般€佸皝闈€佸娉級鍚楋紵' : 'Reset all custom info for this model?')) return;
            document.body.removeChild(modal);
            try {
        ...
        const cancelBtn = document.createElement('button');
        cancelBtn.innerText = window.anomalous_browser_lang === 'zh' ? '鍙栨秷' : 'Cancel';
        ...
        saveBtn.innerText = window.anomalous_browser_lang === 'zh' ? '淇濆瓨鏇存敼' : 'Save Changes';
        ...
        saveBtn.onclick = () => {
            ...
            saveBtn.innerText = window.anomalous_browser_lang === 'zh' ? '淇濆瓨涓?..' : 'Saving...';
            ...
            fetch('/anomalous/update_metadata', {
                ...
                document.body.removeChild(modal);
                alert((window.anomalous_browser_lang === 'zh' ? '閿欒: ' : 'Error: ') + e);
            });
        };
    ...
    async showGeneratedGallery(model) {
        ...
        if (!overlay) {
            ...
            const closeBtn = document.createElement('button');
            closeBtn.innerHTML = '&#10006; ' + (window.anomalous_browser_lang === 'zh' ? '鍏抽棴鍥惧簱' : 'Close Gallery');
        ...
        title.innerText = (window.anomalous_browser_lang === 'zh' ? '鍘嗗彶鐢熸垚鍥惧簱: ' : 'Generated History: ') + (model.name || model.filename);

        const contentCont = document.getElementById('anomalous-generated-gallery-content');
        contentCont.innerHTML = '';

        const loading = document.createElement('div');
        loading.innerText = window.anomalous_browser_lang === 'zh' ? '鍔犺浇涓紝姝ｅ湪鎵弿鍥剧墖鍏冩暟鎹?..' : 'Loading, scanning metadata...';
        ...
        try {
            ...
            if (!data.images || data.images.length === 0) {
                ...
                emptyMsg.innerText = window.anomalous_browser_lang === 'zh' ? '娌℃湁鎵惧埌浣跨敤姝ゆā鍨嬬敓鎴愮殑鍘嗗彶鍥剧墖銆? : 'No images found generated by this model.';
            ...
            data.images.forEach(img => {
                ...
                setCoverBtn.innerText = window.anomalous_browser_lang === 'zh' ? '璁句负灏侀潰' : 'Set Cover';
                ...
                setCoverBtn.onclick = (e) => {
                    e.stopPropagation();
                    if (!confirm(window.anomalous_browser_lang === 'zh' ? '纭畾灏嗘鍥剧墖璁句负灏侀潰鍚楋紵' : 'Set this image as cover?')) return;
                    ...
                    fetch('/anomalous/set_custom_cover', {
                        ...
                        if (result.status === 'success') {
                            alert(window.anomalous_browser_lang === 'zh' ? '璁剧疆鎴愬姛锛? : 'Cover set successfully!');
                            await this.loadModels();
                            const updatedModel = this.models.find(m => m.filename === model.filename);
                            ...
                            alert((window.anomalous_browser_lang === 'zh' ? '閿欒: ' : 'Error: ') + result.message);
                        }
                    }).catch(err => {
    ...
    showGallerySelectMode(model) {
        ...
        banner.innerHTML = window.anomalous_browser_lang === 'zh'
            ? `姝ｅ湪涓烘ā鍨?<span style="color:#ff0;">${model.filename}</span> 閫夋嫨灏侀潰銆傝鐐瑰嚮涓嬫柟鐨勫浘鐗囥€?button id="anomalous-cancel-select" style="margin-left:15px;color:#000;background:#fff;border:none;padding:2px 8px;border-radius:4px;cursor:pointer;">鍙栨秷</button>`
        ...
        try {
            ...
            if (data.notebooks && data.notebooks.length > 0) {
                ...
                data.notebooks.forEach(nb => {
                    ...
                    item.innerHTML = `<span class="anomalous-nb-item-icon">馃搫&nbsp;</span><span class="anomalous-nb-item-text">${nb.name}</span>`;
                    item.onclick = () => {
    ...
    renderNotebookEditor() {
        ...
        const langs = [
            { v: 'zh-CN', l: '馃嚚馃嚦 涓枃 (zh-CN)' }, { v: 'en', l: '馃嚞馃嚙 English (en)' },
            { v: 'ja', l: '馃嚡馃嚨 鏃ユ湰璇?(ja)' }, { v: 'ko', l: '馃嚢馃嚪 頃滉淡鞏?(ko)' },
            { v: 'fr', l: '馃嚝馃嚪 Fran莽ais (fr)' }, { v: 'de', l: '馃嚛馃嚜 Deutsch (de)' },
            { v: 'es', l: '馃嚜馃嚫 Espa帽ol (es)' }, { v: 'ru', l: '馃嚪馃嚭 袪褍褋褋泻懈泄 (ru)' }
        ];
        langs.forEach(lg => {
            const opt = document.createElement('option');
        ...
        const updateVisualTags = () => {
            ...
            tags.forEach((tag, idx) => {
                ...
                copyL.onclick = (e) => {
                    e.stopPropagation();
                    navigator.clipboard.writeText(tag).then(() => { copyL.innerHTML = '鉁?; setTimeout(() => copyL.innerHTML = '馃搵', 1000); });
                };
                ...
                copyR.onclick = (e) => {
                    e.stopPropagation();
                    navigator.clipboard.writeText(txtR.innerText).then(() => { copyR.innerHTML = '鉁?; setTimeout(() => copyR.innerHTML = '馃搵', 1000); });
                };
        ...
        rawBtn.onclick = () => {
            if (rawArea.style.display === 'none') {
                ...
                rawBtn.innerHTML = '馃憗锔?Done Editing';
                pToolbar.style.display = 'none';
            } else {
                rawArea.style.display = 'none';
                dualPane.style.display = 'flex';
                rawBtn.innerHTML = '馃摑 Edit Raw / Paste';
    ...
    sendNotebookToCanvas() {
        ...
        const data = this.currentNotebook.data || {};
        if (!data.mainModel) {
            alert(window.anomalous_browser_lang === 'zh' ? "璇峰厛閫夋嫨涓€涓富妯″瀷銆? : "Please select a Main Model first.");
            return;
        }
    ...
    initDoctorPanel() {
        ...
        titleEl.innerHTML = `<span style="font-size:24px; filter: drop-shadow(0 0 8px rgba(0,255,204,0.3));">馃┖</span><span style="font-size:18px;font-weight:600;color:#fff;font-family:Inter, sans-serif; letter-spacing: 0.5px;">${window.anomalous_browser_lang === 'zh' ? '鍏ㄥ眬浣撴涓績' : 'Global Health Center'}</span>`;
        ...
        const renderAutoScanToggle = () => {
            ...
            autoScanToggle.innerHTML = window.anomalous_browser_lang === 'zh'
                ? (isAutoEnabled 
                    ? '<span style="font-size:16px;">馃泿锔?/span><span style="font-size:12px;color:#00ffcc;font-weight:500;">ComfyUI 鍚姩鏃惰嚜鍔ㄦ娴嬪苟鏇挎崲缂哄け妯″瀷 [宸插紑鍚痌</span>' 
                    : '<span style="font-size:16px;opacity:0.5;">馃敃</span><span style="font-size:12px;color:#aaa;">ComfyUI 鍚姩鏃惰嚜鍔ㄦ娴嬪苟鏇挎崲缂哄け妯″瀷 [榛樿鍏抽棴]</span>')
                : (isAutoEnabled 
                    ? '<span style="font-size:16px;">馃泿锔?/span><span style="font-size:12px;color:#00ffcc;font-weight:500;">Auto-Detect on ComfyUI Startup [ON]</span>' 
                    : '<span style="font-size:16px;opacity:0.5;">馃敃</span><span style="font-size:12px;color:#aaa;">Auto-Detect on ComfyUI Startup [OFF]</span>');
        };
    ...
    diagnoseNode(node) {
        ...
        if (!node) {
            placeholder.innerHTML = `<div style="font-size:48px;">馃</div><div style="text-align:center;">${window.anomalous_browser_lang === 'zh' ? '璇峰湪鐢诲竷涓?strong style="color:#aaa">鐐瑰嚮閫変腑浠绘剰鑺傜偣</strong>' : 'Please <strong style="color:#aaa">select any node</strong> in the canvas'}</div>`;
        ...
        if (modelWidgets.length === 0) {
            placeholder.innerHTML = `<div style="font-size:36px;">鈿狅笍</div><div style="text-align:center;">${window.anomalous_browser_lang === 'zh' ? '璇ヨ妭鐐规病鏈夊彈鏀寔鐨勬ā鍨嬪弬鏁? : 'No supported model parameter on this node'}</div><div style="font-size:12px;color:#555;margin-top:4px;">${node.type || ''}</div>`;
            placeholder.style.display = 'flex';
            nodeContent.style.display = 'none';
            nodeContent.innerHTML = '';
            return;
        ...
        titleBar.innerHTML = `<span style="font-size:18px;">馃</span><span style="font-weight:bold;color:#fff;font-size:14px;">${node.title || node.type || 'Node'}</span><span style="font-size:11px;color:#555;margin-left:auto;">${node.type || ''}</span>`;
    ...
    renderGlobalDashboard() {
        ...
        statsRow.innerHTML = `
            ${createBadge(zh?'鎬绘ā鍨?:'Total', total, '#aaa', 'rgba(255,255,255,0.05)')}
            ${createBadge(zh?'馃煝 鍋ュ悍':'馃煝 Healthy', healthy, '#28a745', 'rgba(40, 167, 69, 0.1)')}
            ${createBadge(zh?'馃敶 缂哄け':'馃敶 Missing', missing, '#ff6b6b', 'rgba(220, 53, 69, 0.1)')}
        `;
        ...
        if (total === 0) {
            content.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:rgba(255,255,255,0.3);font-size:14px;">
                <div style="font-size:48px;margin-bottom:16px;">馃懟</div>
                ${zh ? '褰撳墠宸ヤ綔娴佹病鏈変换浣曟ā鍨嬭妭鐐? : 'No models found in this workflow'}
            </div>`;
            return;
        }
        ...
        for (const data of missingNodesData) {
            ...
            if (isHealthy && exactMatch && exactMatch !== val) {
                right.innerHTML = `<div style="text-align:right;"><div style="color:#ffc107;font-size:13px;font-weight:bold;">馃煛 ${zh?'鑷姩閲嶅畾鍚?:'Auto-Redirected'}</div><div style="color:rgba(255,255,255,0.4);font-size:11px;margin-top:4px;">${exactMatch.split(/[\/]/).pop()}</div></div>`;
            } else if (isHealthy) {
                right.innerHTML = `<div style="color:#28a745;font-size:13px;font-weight:bold;padding:6px 12px;background:rgba(40,167,69,0.1);border-radius:20px;">馃煝 ${zh?'姝ｅ父':'Ready'}</div>`;
            } else {
                right.innerHTML = `<div style="color:#ff6b6b;font-size:13px;font-weight:bold;padding:6px 12px;background:rgba(220,53,69,0.1);border-radius:20px;">馃敶 ${zh?'涓㈠け':'Missing'}</div>`;
            }
            ...
            civitaiBtn.innerHTML = zh ? '馃寪 C绔? : '馃寪 Civitai';
            ...
            if (!isHealthy) {
                ...
                deepScanBtn.innerHTML = zh ? '馃攳 娣卞害鍝堝笇鎵弿' : '馃攳 Deep Hash Scan';
                ...
                deepScanBtn.onclick = async () => {
                    deepScanBtn.innerText = zh ? '鈴?姝ｅ湪鍚姩鎵弿寮曟搸...' : '鈴?Starting scan engine...';
                    ...
                    try {
                            ...
                            try {
                                ...
                                if (statusData.scanning) {
                                    ...
                                    deepScanBtn.innerText = zh 
                                        ? `鈴?鎵弿涓?(${statusData.current}/${statusData.total}) ${filename}`
                                        : `鈴?Scanning (${statusData.current}/${statusData.total}) ${filename}`;
                                } else {
                                    clearInterval(checkInterval);
                                    if (statusData.error) {
                                        alert(zh ? '鉂?鎵弿杩囩▼涓彂鐢熼敊璇? ' + statusData.error : '鉂?Scan error: ' + statusData.error);
                                    }
                                    
                                    deepScanBtn.innerText = zh ? '鈴?姝ｅ湪鍖归厤骞舵浛鎹㈤绾㈣妭鐐?..' : '鈴?Matching and resolving red nodes...';
                                    ...
                                    const scanInfo = zh 
                                        ? `鍏辨繁搴︽壂鎻忎簡 ${statusData.total} 涓己澶变俊鎭殑妯″瀷銆俙
                                        : `Deep scanned ${statusData.total} models with missing info.`;
                                        
                                    if (stillMissing) {
                                        alert(zh ? `鉂?鎵弿缁撴潫锛屾湰鍦版湭鍖归厤鍒版ā鍨嬨€俓n\n${scanInfo}\n\n杩欒鏄庢ā鍨嬪彲鑳界湡鐨勪笉鍦ㄦ偍鐨勭‖鐩橀噷锛屾垨鑰呮偍鍒犻櫎浜嗗師鏈褰曠潃鍝堝笇鐨勬簮鏂囦欢銆俓n璇风偣鍑诲崱鐗囦笂鐨勩€愷煂?C绔欍€戝幓浜戠涓嬭浇銆俙 : `鉂?Scan finished, but no local match found.\n\n${scanInfo}\n\nThis means the model is truly missing from your disk, or the original source file with hash was deleted.\nPlease click [馃寪 Civitai] to download it from the cloud.`);
                                    } else {
                                        alert(zh ? `鉁?娣卞害鎵弿鎴愬姛锛佸凡鑷姩淇璇ヨ妭鐐癸紒\n\n${scanInfo}` : `鉁?Deep Scan successful! Node auto-healed!\n\n${scanInfo}`);
                                    }
                        ...
                        alert(zh ? '鉂?鎵弿鍑洪敊: ' + e.message : '鉂?Scan Error: ' + e.message);
                        deepScanBtn.innerHTML = zh ? '馃攳 娣卞害鍝堝笇鎵弿' : '馃攳 Deep Hash Scan';
                        deepScanBtn.disabled = false;
                        deepScanBtn.style.opacity = '1';
                    }
                ...
                manualBtn.innerHTML = zh ? '馃攢 鎵嬪姩鏇挎崲' : '馃攢 Manual Replace';
                ...
                pathText.innerText = `${zh?'鍘熻矾寰?:'Original'}: ${val}`;
    ...
    initAssistantPanel() {
        ...
        placeholder.innerHTML = `<div style="font-size:48px;">馃</div><div>${window.anomalous_browser_lang === 'zh' ? '璇峰湪鐢诲竷涓?strong style="color:#aaa">鐐瑰嚮閫変腑浠绘剰鑺傜偣</strong>' : 'Please <strong style="color:#aaa">select any node</strong> in the canvas'}</div>`;
    ...
    renderAssistantModelCard(node, w, container) {
        ...
        previewBox.innerHTML = `<span style="color:#444;font-size:13px;">${window.anomalous_browser_lang === 'zh' ? '鍔犺浇棰勮...' : 'Loading preview...'}</span>`;
        ...
        browseBtn.innerText = window.anomalous_browser_lang === 'zh' ? '馃攢 鏇存崲妯″瀷' : '馃攢 Change Model';
        ...
        profileBtn.innerText = window.anomalous_browser_lang === 'zh' ? '馃摉 鏌ョ湅妗ｆ' : '馃摉 View Profile';
        ...
        fetch(`/anomalous/find_model?search=${encodeURIComponent(val.replace(/\\/g, '/'))}`)
            ...
            .then(d => {
                ...
                if (d.status === 'success' && d.model && d.model.preview_url) {
                    ...
                    previewBox.innerHTML = `<span style="color:#444;font-size:13px;">${window.anomalous_browser_lang === 'zh' ? '鏆傛棤棰勮鍥? : 'No preview'}</span>`;
                }
                ...
                if (d.status === 'success' && d.model && d.model.metadata) {
                    ...
                    if (triggers && triggers.length > 0) {
                        ...
                        trigTitle.innerText = window.anomalous_browser_lang === 'zh' ? '瑙﹀彂璇? : 'Trigger Words';
                        ...
                        words.forEach(word => {
                            ...
                            tag.innerText = word;
                            tag.title = window.anomalous_browser_lang === 'zh' ? '鐐瑰嚮澶嶅埗' : 'Click to copy';
                            tag.onclick = () => {
                                navigator.clipboard.writeText(word).then(() => { const o = tag.innerText; tag.innerText = '鉁?; setTimeout(() => tag.innerText = o, 1000); });
                            };
                        ...
                        copyAll.innerText = window.anomalous_browser_lang === 'zh' ? '馃搵 鍏ㄩ儴澶嶅埗' : '馃搵 Copy All';
                        copyAll.onclick = () => {
                            navigator.clipboard.writeText(words.join(', ')).then(() => { copyAll.innerText = '鉁?; setTimeout(() => copyAll.innerText = window.anomalous_browser_lang === 'zh' ? '馃搵 鍏ㄩ儴澶嶅埗' : '馃搵 Copy All', 1500); });
                        };
                    ...
                    if (textNotes) {
                        ...
                        notesTitle.innerText = window.anomalous_browser_lang === 'zh' ? '馃摑 鎴戠殑澶囨敞' : '馃摑 My Notes';
                        const notesText = document.createElement('div');
                ...
                // History gallery 鈥?always load if we can resolve the filename
                ...
                previewBox.innerHTML = `<span style="color:#444;font-size:13px;">${window.anomalous_browser_lang === 'zh' ? '鏃犳硶鍔犺浇棰勮' : 'Failed to load preview'}</span>`;
    ...
    _loadAssistantHistory(filename, container, model) {
        fetch('/anomalous/model_images?model_name=' + encodeURIComponent(filename) + '&t=' + Date.now())
            ...
            .then(data => {
                ...
                sectionTitle.innerText = window.anomalous_browser_lang === 'zh' ? `馃柤锔?鍘嗗彶鐢熸垚鍥?(${images.length})` : `馃柤锔?History (${images.length})`;
                sectionHeader.appendChild(sectionTitle);

                // "View all" button if model is available
                if (model) {
                    const viewAllBtn = document.createElement('button');
                    viewAllBtn.innerText = window.anomalous_browser_lang === 'zh' ? '鏌ョ湅鍏ㄩ儴 鈫? : 'View All 鈫?;
                ...
                images.slice(0, 16).forEach(img => {
                    ...
                    if (img.workflow) {
                        card.title = window.anomalous_browser_lang === 'zh' ? '鐐瑰嚮鎭㈠姝ゅ伐浣滄祦' : 'Click to restore workflow';
                        card.onclick = () => {
                            try {
                        ...
                        card.title = window.anomalous_browser_lang === 'zh' ? '鐐瑰嚮鏌ョ湅瀹屾暣鍥惧簱' : 'Click to view full gallery';
    ...
    _openGalleryReplacer(node, w) {
        ...
        title.innerText = window.anomalous_browser_lang === 'zh' ? '馃柤锔?閫夋嫨瑕佹浛鎹㈢殑妯″瀷' : '馃柤锔?Select Replacement Model';
        ...
        fetch('/anomalous/resolve_paths_to_previews', {
            ...
            for (const path of validPaths) {
                ...
                if (previews[path]) { imgBox.style.backgroundImage = `url("${previews[path]}")`; imgBox.innerText = ''; } else { imgBox.innerText = '馃柤锔?; }
    ...
    runGlobalDoctorScan() {
                ...
                for (let w of node.widgets) {
                    ...
                    if (typeof val === 'string' && (val.endsWith('.safetensors') || val.endsWith('.ckpt') || val.endsWith('.pt') || val.endsWith('.sft') || val.endsWith('.bin'))) {
                        ...
                        if (!isHealthy) {
                            missingNodes++;
                            const nodeTitle = document.createElement('div');
                            nodeTitle.innerText = `${window.anomalous_browser_lang === 'zh' ? '鑺傜偣' : 'Node'}: ${node.title || node.type}`;
        ...
        if (missingNodes === 0) {
            content.innerHTML = `<div style="color:#28a745; text-align:center; padding:20px; font-size:16px;">${window.anomalous_browser_lang === 'zh' ? '馃帀 澶浜嗭紒褰撳墠宸ヤ綔娴佷腑鎵€鏈夋ā鍨嬪潎鍦ㄦ湰鍦板氨缁€? : '馃帀 Awesome! All models in this workflow are healthy.'}</div>`;
        }
    }
    ...
    showPreflightImportModal() {
        ...
        if (!overlay) {
            ...
            header.innerText = window.anomalous_browser_lang === 'zh' ? '馃摜 瀵煎叆宸ヤ綔娴?(棰勬娴峰叧)' : '馃摜 Pre-flight Import Workflow';
            modal.appendChild(header);

            const desc = document.createElement('div');
            desc.style.color = '#aaa';
            desc.style.fontSize = '14px';
            desc.innerText = window.anomalous_browser_lang === 'zh' ? '璇峰湪姝ゅ绮樿创宸ヤ綔娴佺殑 JSON 浠ｇ爜锛堟湭鏉ュ皢鏀寔鍒嗕韩鐮侊級銆傛垜浠皢涓烘偍鎻愬彇骞舵牎楠屾墍鏈夋ā鍨嬩緷璧栥€? : 'Paste the workflow JSON here. We will extract and validate all model dependencies before loading.';
            ...
            analyzeBtn.innerText = window.anomalous_browser_lang === 'zh' ? '馃攳 瑙ｆ瀽骞堕妫€' : '馃攳 Analyze & Pre-flight';
            ...
            loadBtn.innerText = window.anomalous_browser_lang === 'zh' ? '馃殌 寮哄埗杞藉叆鐢诲竷' : '馃殌 Force Load to Canvas';
            ...
            analyzeBtn.onclick = async () => {
                try {
                    ...
                    if (models.length === 0) {
                        resultArea.innerHTML = `<div style="color:#28a745;">${window.anomalous_browser_lang === 'zh' ? '鉁?鏈娴嬪埌浠讳綍妯″瀷渚濊禆銆? : '鉁?No model dependencies detected.'}</div>`;
                        return;
                    }

                    resultArea.innerHTML = `<div style="color:#fff; font-weight:bold; margin-bottom:10px;">${window.anomalous_browser_lang === 'zh' ? `鎵弿鍒?${models.length} 涓ā鍨嬭祫婧愶細` : `Detected ${models.length} model resources:`}</div>`;
                    ...
                    for (const m of models) {
                        ...
                        icon.innerText = isHealthy ? '鉁? : '鈿狅笍';
                        ...
                        if (!isHealthy) {
                            ...
                            right.target = '_blank';
                            right.innerText = window.anomalous_browser_lang === 'zh' ? '鍘?C绔欎笅杞? : 'Download';
                            ...
                            right.innerText = window.anomalous_browser_lang === 'zh' ? '宸插氨缁? : 'Ready';
...
app.registerExtension({
    ...
    async setup() {
        ...
        btn.innerHTML = '馃摝';
        btn.title = window.anomalous_browser_lang === 'zh' ? '鎵撳紑 Anomalous 妯″瀷娴忚鍣? : 'Open Anomalous Model Browser';
...
const AMB_WorkflowShare = {
    ...
    showExportModal() {
        ...
        const title = document.createElement('h2');
        title.style.margin = '0';
        title.textContent = lang === 'zh' ? '馃摝 瀵煎嚭宸ヤ綔娴佸垎浜爜' : '馃摝 Export Workflow Share Code';
        ...
        typeSelectContainer.innerHTML = `
            <label style="display: block; margin-bottom: 8px; cursor: pointer;">
                ...
                ${lang === 'zh' ? '绮剧畝鐗?(鎺ㄨ崘 B绔欒瘎璁哄尯, 浣撶Н灏? 涓㈠け鍧愭爣)' : 'Skeleton (Compact, best for comments, loses coords)'}
            </label>
            <label style="display: block; cursor: pointer;">
                <input type="radio" name="amb-share-type" value="full" />
                ${lang === 'zh' ? '瀹屾暣鐗?(淇濈暀鎵€鏈夊潗鏍囦笌棰滆壊, 杈冮暱)' : 'Full (Keeps coords and colors, longer string)'}
            </label>
        `;
        ...
        generateBtn.textContent = lang === 'zh' ? '鐢熸垚鍒嗕韩鐮? : 'Generate';
        ...
        copyBtn.textContent = lang === 'zh' ? '澶嶅埗鍒板壀璐存澘' : 'Copy';
        ...
        const closeBtn = document.createElement('button');
        closeBtn.textContent = lang === 'zh' ? '鍏抽棴' : 'Close';
        ...
        copyBtn.onclick = () => {
            ...
            alert(lang === 'zh' ? '澶嶅埗鎴愬姛锛? : 'Copied successfully!');
        };
    ...
    showImportModal() {
        ...
        const title = document.createElement('h2');
        title.style.margin = '0';
        title.textContent = lang === 'zh' ? '馃摜 瀵煎叆宸ヤ綔娴佸垎浜爜' : '馃摜 Import Workflow Share Code';
        
        const inputArea = document.createElement('textarea');
        inputArea.placeholder = lang === 'zh' ? '绮樿创 AMB0- 鎴?AMB1- 寮€澶寸殑鍒嗕韩鐮?..' : 'Paste AMB0- or AMB1- share code here...';
        ...
        const closeBtn = document.createElement('button');
        closeBtn.textContent = lang === 'zh' ? '鍙栨秷' : 'Cancel';
        closeBtn.style.cssText = `padding: 8px 16px; background: #555; color: #fff; border: none; border-radius: 6px; cursor: pointer;`;
        ...
        previewBtn.onclick = async () => {
            ...
            try {
                ...
                let html = `<strong>${lang === 'zh' ? '宸ヤ綔娴佹瑙? : 'Workflow Overview'}</strong><br>`;
                html += `${lang === 'zh' ? '鎬昏妭鐐规暟' : 'Total Nodes'}: ${nodesCount}<br><br>`;
                
                if (missingNodes.length > 0) {
                    html += `<span style="color: #ff6b6b;">鈿狅笍 ${lang === 'zh' ? '缂哄け浠ヤ笅鑺傜偣 (瀵煎叆鍚庡皢鏄剧ず绾㈡)' : 'Missing Node Types (will be red on canvas)'}:</span><br>`;
                    ...
                    html += `<span style="color: #5cb85c;">鉁?${lang === 'zh' ? '鎵€鏈夋墍闇€鑺傜偣宸插湪褰撳墠鐜瀹夎锛? : 'All required nodes are installed!'}</span>`;
                }
                ...
                previewArea.style.display = 'block';
                previewArea.innerHTML = `<span style="color:#ff6b6b;">${lang === 'zh' ? '瑙ｆ瀽澶辫触' : 'Decode Failed'}: ${err.message}</span>`;
            }
        };
        ...
        const title = document.createElement('h2');
        title.style.margin = '0';
        title.textContent = lang === 'zh' ? '馃攧 宸ヤ綔娴佸垎浜笌瀵煎叆' : '馃攧 Workflow Share & Import';
        
        const exportBtn = document.createElement('button');
        exportBtn.textContent = lang === 'zh' ? '馃摛 瀵煎嚭褰撳墠宸ヤ綔娴佷负鍒嗕韩鐮? : '馃摛 Export Workflow to Share Code';
        ...
        const importBtn = document.createElement('button');
        importBtn.textContent = lang === 'zh' ? '馃摜 浠庡垎浜爜瀵煎叆宸ヤ綔娴? : '馃摜 Import Workflow from Share Code';
