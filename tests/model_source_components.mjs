...
await f.flush();
const rows = f.document.body.querySelectorAll('.anomalous-source-row-item');
...
const pending = all(missing).find(el => el.textContent === 'Source needed' && el.className === 'anomalous-source-badge-neutral');
