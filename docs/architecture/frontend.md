...
floating trigger button is positioned via pure responsive CSS in the safe top-right canvas area (`top: 80px; right: 24px;`),
comfortably below ComfyUI's dual-row top action and workflow bars (~64px) and completely clear of the left Dock sidebar (~48px)
without inline left/top overrides. `browser_entry.js` utilizes modern PointerEvents with `setPointerCapture` and strictly distinguishes clicks from
...
Resetting trigger position cleanly restores default CSS top-right positioning.
