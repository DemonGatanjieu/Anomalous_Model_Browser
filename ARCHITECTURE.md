...
- `ui_sidebar.js` creates the browser shell and folder navigation.
  ...
  Scan-wizard launch, single-model precision scans (`triggerDirectModelScan` with strictly factual Civitai vs non-Civitai feedback reporting inferred base-model or match status directly within the bottom-right progress panel and toasts without blocking browser alerts), and polling live in `ui_scan_wizard.js`; folder visibility/order lives in
...
- `ui_materials.js`, `ui_material_cards.js`, and `ui_material_application.js` own the Material Library UI, category navigation, card presentation (with grab cursor affordances and explicit drag tooltips), context-aware drag guidance, and the structured empty state onboarding blueprint guiding users through collection, canvas drag, and prompt studio mixing.
...
- `ui_recipes.js` / `ui_recipe_detail.js`, `ui_notebooks.js`, and `ui_materials.js`
  ...
  filters and navigation, `ui_recipe_cards.js` owns cards and card actions,
  `ui_recipe_dialogs.js` owns save/edit dialogs, and `ui_recipe_media.js` owns shared cover helpers.
