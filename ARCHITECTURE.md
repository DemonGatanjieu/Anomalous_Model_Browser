...
- `ui_sidebar.js`'s Toolbox modal strictly filters out all tools already present on the bottom bar, presenting a sleek 216px auto-height utility catalog with compact, frameless 40px tiles (transparent normal state, hover pill highlight, 13.5px icons), clean click action execution, and zero obstructive tooltip intrusion while open.
