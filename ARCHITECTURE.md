...
The browser has three entry modes backed by one shared `openBrowser()` command: the customizable floating trigger, a ComfyUI action-bar button, or the command under `Extensions -> Anomalous Model Browser` alone. The command also owns the native `Ctrl + Shift + M` default keybinding, allowing ComfyUI to expose user remapping, duplicate detection, reserved-key guidance, and modal guards without a plugin-level keyboard listener. One native combo setting makes the visual presentations strictly exclusive; temporary host redraws must never override the saved mode or reveal a second entry. Floating size is bounded to small/standard/large and appearance to icon/pill; their setting rows are conditionally hidden outside floating mode because they have no effect there. The colocated language preference uses `auto` by default to resolve `Comfy.Locale`, with explicit `zh`/`en` plugin-only overrides. The in-browser language shortcut writes through the same native setting instead of maintaining an independent preference. The action-bar button uses the host's extension-level `actionBarButtons` contract instead of locating or rewriting another component's DOM. Opening the main browser hides only the floating trigger, while closing restores it only in floating mode. The Extensions command remains registered in every mode as a recovery path, and all modes reuse the same browser instance.

The Interface settings render the active browser shortcut from ComfyUI's registered command object. Its customization button navigates to the native keybinding panel, filters the shared open command, and launches the host's own recorder; the plugin does not write a parallel shortcut preference. Language changes replace the registered Interface setting descriptors with freshly translated copies so ComfyUI's reactive settings tree immediately updates the open category heading, labels, tooltips, custom controls, and combo options without a page refresh. **Design direction, not a roadmap:** after the browser command surface and individual features are stable, focused commands for selected high-value actions may use the same native binding pattern. This is guidance for future command design, not a commitment to add bindings or a scheduled feature set.
