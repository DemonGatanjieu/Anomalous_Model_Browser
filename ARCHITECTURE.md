...
- `ui_sidebar.js`'s Toolbox modal strictly filters out all tools already present on the bottom bar, presenting a compact, scaled-down 3-column utility catalog with 9-item capacity (currently holding 4: `workflow-transfer`, `prompt-studio`, `prompt-translator`, `model-sources`) with 16px color emoji icons and harmonious proportions with the bottom toolbar.
