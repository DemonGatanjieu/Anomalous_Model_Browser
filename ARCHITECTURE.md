...
- `ui_sidebar.js`'s Toolbox modal strictly filters out all tools already present on the bottom bar, presenting a compact, auto-height 3-column utility catalog with 48px cards, refined 14.5px color emoji icons, subtle expansion placeholders (`＋ 待扩充`), clean click action execution, and zero obstructive tooltip intrusion while open.
